...
impl TableData {
    ...
    /// Sets one column's explicit width fraction, initializing every column
    /// to an equal share first if no explicit widths exist yet, then
    /// renormalizing the whole row so fractions keep summing to ~1.0. This is
    /// the only place that produces `Some` widths, so it is what keeps
    /// `widths.len() == alignments.len()` — later rounds call it from the
    /// mutation API and drag-to-resize.
    pub fn set_column_width(&mut self, column: usize, fraction: f32) {
        self.normalize_shape();
        let columns = self.column_count();
        if column >= columns {
            return;
        ...
        widths[column] = fraction.max(0.0);
...
mod tests {
    ...
    #[test]
    fn set_column_width_keeps_widths_length_in_sync_with_alignments() {
        let mut table = TableData::new_empty(1, 3);
        table.set_column_width(1, 0.6);
        let widths = table.widths.as_ref().expect("widths should be Some");
        assert_eq!(widths.len(), table.alignments.len());
        assert_close(widths.iter().sum::<f32>(), 1.0);

        // A second call reuses (rather than resets) the existing widths.
        table.set_column_width(0, 0.1);
        let widths = table.widths.as_ref().expect("widths should be Some");
        assert_eq!(widths.len(), table.alignments.len());
        assert_close(widths.iter().sum::<f32>(), 1.0);
    }
